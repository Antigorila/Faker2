    ...
    internal class Program
    {
        ...
        class SQL
        {
            ...
            static public void DeleteTable(string promt)
            {
                ...
                con.Open();
                string deleteTableQuery = $"USE {SQL.database}; DROP TABLE {tableName};";
            ...
            static public void CreateTable(string promt)
            {
                ...
                while (true)
                {
                    ...
                    else if (SQL.Tables().Contains(tablename))
                    {
                ...
                string CreateTableCommand = $"USE {SQL.database}; CREATE TABLE {tablename} (";
            ...
            private static Dictionary<string, string> ColumnsAndTypesOfTable(string tablename)
            {
                ...
                List<string[]> lis = SqlQuery($"USE {SQL.database}; SHOW COLUMNS FROM {tablename};");
                for (int i = 0; i < lis.Count; i++)
                {
            ...
            static public void FakerTable(string promt)
            {
                ...
                try
                {
                    ...
                    {
                        string insertData = $"USE {SQL.database}; INSERT INTO {tableName} (";
        ...
        static void Main(string[] args)
        {
            while (true)
            {
                ConsoleMessage(@"Faker\0.1W>", "Enter the database name: ");
                string databaseName = Console.ReadLine();
                if (!SQL.ContainsIllegalRegex(databaseName))
                {
                    SQL.SetDatabaseName(databaseName);
                    ConsoleMessageLine(@"Faker\0.1W\if2>", "Database name approved");
                    break;
                }
            }
            
            ConsoleMessageLine(@"Faker\1>", "Enter the commands to use the Faker. If you need any help just type --help. If you done just enter 'stop'");
            while (true)
            {
                ConsoleMessage(@"Faker\1.1W\EnterCommand> ", "");
                string input = Console.ReadLine();
                if (input == "--help")
                {
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can create a table with: 'Create table' command");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can delete a table with: 'Delete table' command");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can list out the tables with: 'List table' command");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can list out the Dummy Data types with: 'Dummys' command and then you can list out its datas");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can create new Dummy types and fill them with datas by yourself with: 'Create Dummy'");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can expand existing Dummy Data types or delet them with 'Extends Dummy'");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "You can fake dates with Dummy Datas in specific tables and columns with 'Fake'");
                    ConsoleMessageLine(@"Faker\1.1W\if1>", "To shut down just enter 'stop'");
                }
                else if (input.ToLower() == "stop")
                {
                    ConsoleMessageLine(@"Faker\1.1W\if2>", "Shutdown...");
                    break;
                }
                else if (input.ToLower().Contains("create table"))
                {
                    SQL.CreateTable(@"Faker\1.1W\if3>");
                    ConsoleMessageLine(@"Faker\1.1W\if3>", "Table and data creation has ran down");
                }
                else if (input.ToLower().Contains("delete table"))
                {
                    SQL.DeleteTable(@"Faker\1.1W\if4>");
                    ConsoleMessageLine(@"Faker\1.1W\if4>", "Table purge has ran dwon");
                }
                else if (input.ToLower().Contains("list table"))
                {
                    SQL.ShowTables(@"Faker\1.1W\if5>");
                    ConsoleMessageLine(@"Faker\1.1W\if5>", "Table list has ran down");
                }
                else if (input.ToLower().Contains("dummys"))
                {
                    SQL.DummyTypes(@"Faker\1.1W\if6>");
                    ConsoleMessageLine(@"Faker\1.1W\if6>", "View Dummy Datas has ran down");
                }
                else if (input.ToLower().Contains("create dummy"))
                {
                    SQL.CreateNewDummyDataTypes(@"Faker\1.1W\if7>");
                    ConsoleMessageLine(@"Faker\1.1W\if7>", "Creat Dummy Data has ran down");
                }
                else if (input.ToLower().Contains("extends dummy"))
                {
                    SQL.CreateNewDummyDataTypes(@"Faker\1.1W\if8>");
                    ConsoleMessageLine(@"Faker\1.1W\if8>", "Extends Dummy Data has ran down");
                }
                else if (input.ToLower().Contains("fake"))
                {
                    SQL.FakerTable(@"Faker\1.1W\if9>");
                    ConsoleMessageLine(@"Faker\1.1W\if9>", "Fake table with Dummy Datas has ran down");
                }
            }
            //Console.ReadKey();
        }
